...
class ProveedorCapasCopernicus extends ProveedorCapas {
    constructor(opciones) {
        ...
        config.vectores.forEach(vector => {
            let opciones = {
                formatos:{
                    isolineas:true, isobandas:true, serieTiempo:true, valorEnPunto:true, matrizRectangular:true, windglPNG:true, uv:true
                },
    ...
    getMetadata(tiempo, producto) {
        return new Promise((resolve, reject) => {            
            ...
            //console.log("buscando " + path + ".metadata");
    ...
    async getPreconsulta(codigoCapa, lng0, lat0, lng1, lat1, tiempo, nivel, maxWidth, maxHeight) {
        try {
            ...
            await gdal.translateWindow(bbox.lng0, bbox.lat0, bbox.lng1, bbox.lat1, srcPath, outPath, codigoCapa, (width || height)?{width:width, height:height}:null, "bilinear");
    ...
    async generaSerieTiempo(args) {
        try {
            ...
            let lat = args.lat;
            let lng = args.lng;
            let time0 = this.normalizaTiempoProducto(moment.tz(args.time0, "UTC"), producto);
            let time1 = this.normalizaTiempoProducto(moment.tz(args.time1, "UTC"), producto);
    ...
    iniciaExtraccionSiguientePuntoSerieTiempo(puntosPendientes, puntosAgregados, control) {
        if (puntosPendientes.length) {
            ...
            gdal.getPointValue(args.lng, args.lat, args.path, args.subDataset, args.tmpPath)
                .then(punto => {
                    punto = this.aplicaTransformacion(args.subDataset, punto);
                    let atributos = {
    ...
    async generaValorEnPunto(args) {
        try {
            ...
            if (varMetadata) {
                ...
                let path = this.getPathProducto(time, producto) + ".nc";
                let punto = await gdal.getPointValue(lng, lat, path, args.codigoVariable, config.dataPath + "/tmp");
    ...
    async generaMatrizUV(args) {
        try {
            ...
            let ret = {
                ...
                deltaLng:(matrizU.lng1 - matrizU.lng0) / args.resolution,
                deltaLat:(matrizU.lat1 - matrizU.lat0) / args.resolution,
    ...
    async generaWindGLPNG(args) {
        try {
            ...
            args.resolutionX = width; 
            args.resolutionY = height;           
            ...
            let ret = {
                lat0:matrizU.lat0, lat1:matrizU.lat1, lng0:matrizU.lng0, lng1:matrizU.lng1, 
                time:matrizU.time,
            ...
            let u=[], v=[];
            matrizU.rows.forEach(row => row.forEach(value => u.push(value)));
            matrizV.rows.forEach(row => row.forEach(value => v.push(value)));
    ...
    async generaMatrizRectangular(args) {
        try { 
            if (this.esVector(args.codigoVariable)) return await this.generaMatrizRectangularVector(args);
            let capa = this.getCapa(args.codigoVariable);
            if (!capa) throw "No se encontró la variable '" + args.codigoVariable + "'";
            let levelIndex = 0;
            if (args.levelIndex) levelIndex = args.levelIndex;
            ...
            let maxWidth = args.maxWidth || 250;
            let maxHeight = args.maxHeight || 250;
            let res = 0.05, width = undefined, height = undefined; 
            if (args.resolution) {
                width = args.resolution;
                height = args.resolution;
            } else if (args.resolutionX && args.resolutionY) {
                width = args.resolutionX;
                height = args.resolutionY;
            } else {
                if ((b.lng1 - b.lng0) / res > maxWidth) {
                    width = maxWidth;
                    height = (b.lat1 - b.lat0) / res;
                }
                if ((b.lat1 - b.lat0) / res > maxHeight) {
                    height = maxHeight;
                    width = width || (b.lng1 - b.lng0) / res;
                }    
            }        

            let path = this.getPathProducto(time, producto) + ".nc";
            let data = await gdal.getRectangularMatrix(b.lng0, b.lat0, b.lng1, b.lat1, path, args.codigoVariable, width, height, config.publishPath, "bilinear");
    ...
    async generaMatrizRectangularVector(args) {
        try { 
            let vector = this.getVector(args.codigoVariable);
            let argsU = JSON.parse(JSON.stringify(args));
            argsU.codigoVariable = vector.vector.capaU;
