...
class ProveedorCapasCopernicus extends ProveedorCapas {
    ...
    async generaMatrizRegular(args) {
        try {
            ...
            if (!varMetadata) return null;
            let b = this.normalizaBBox(args.lng0, args.lat0, args.lng1, args.lat1, varMetadata);
    ...
    async generaMatrizRectangular(args, interpolacion) {
        try { 
            ...
            let path = this.getPathProducto(time, producto) + ".nc";
            let {data, box} = await gdal.getRectangularMatrix(b.lng0, b.lat0, b.lng1, b.lat1, path, args.codigoVariable, width, height, config.publishPath, interpolacion, varMetadata);
            ...
            data.lng0 = box.newLng0; data.lng1 = box.newLng1;
            data.lat0 = box.newLat0; data.lat1 = box.newLat1;
