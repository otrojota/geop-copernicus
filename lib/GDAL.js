...
class GDAL {
    ...
    */
    ...
    getRectangularMatrix(lng0, lat0, lng1, lat1, file, subDataset, width, height, tmpPath, interpolacion) {
        return new Promise((resolve, reject) => {
            let tmpFileName = tmpPath + "/tmp_" + parseInt(1000000 * Math.random());
            let cmd = "gdal_translate -q -projwin " +lng0 + " " + lat1 + " " + lng1 + " " + lat0;
    ...
    getPixelValue(lng, lat, file, subDataset, varMetadata) {
        ...
        // TODO: Calcular usando gdalinfo si no están en la metadata (datos viejos)
