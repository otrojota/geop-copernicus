...
class MOTUClient {
    ...
    download(producto, fecha, outDir, outName) {
        return new Promise((resolve, reject) => {            
            let cmd = "";
            cmd += `python  -m motuclient --motu ${producto.url} --service-id ${producto.serviceId} --product-id ${producto.productId}`;
            cmd += `        --longitude-min ${config.limites.w} --longitude-max ${config.limites.e}`;
            cmd += `        --latitude-min ${config.limites.s} --latitude-max ${config.limites.n}`;
            cmd += `        --date-min "${fecha}" --date-max "${fecha}"`;
            cmd += `        ${producto.variables.reduce((st, v) => (st + (st?" ":"") + "--variable " + v.codigo), "")}`;
